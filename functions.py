...
def searcher(json):
    ...
    params_1 = {
        'fly_from': json["Origin"][0],
        'date_from': json["Departure_d"][0],
        'date_to': json["Arrivals_d"][0],
        'dtime_from': json["Departures_h"][0],
        'dtime_to': "23:00",
        'return_from': json["Origin"][0],
        'return_to': json["Arrivals_h"][0],
        'ret_atime_from': "06:00",
        'ret_atime_to': json["Arrivals_h"][0],
        'adults': json["Travelers"][0],
        'max_stopovers': json["Origin"][0],
        'one_for_city': 1,
    ...
    params_2 = {
        ...
        'date_to': json["Departure_d"][1],
        'dtime_from': json["Departures_h"][1],
        'dtime_to': "23:59",
        'return_from': json["Arrivals_d"][1],
        'return_to': json["Arrivals_d"][1],
    ...
    from datetime import datetime
     ...
     #Origin_1_from
    original_date_string = f"{json['Departure_d'][0]}T{json['Departures_h'][0]}:00.00Z"
    ...
    time_from_1 = original_datetime.strftime(new_date_format)
     ...
     #Origin_1_to
    original_date_string = f"{json['Arrivals_d'][0]}T{json['Arrivals_h'][0]}:00.00Z"
    original_datetime = datetime.strptime(original_date_string, "%d/%m/%YT%H:%M:%S.%fZ")
    new_date_format = "%Y-%m-%dT%H:%M:%S.%fZ"
    ...
    data_1['data'].append({"id": "null", "flyFrom": f"{json['Origin'][0]}", "flyTo": f"{json['Origin'][0]}", "cityFrom": f"{json['Origin'][0]}",
                           "cityCodeFrom": f"{json['Origin'][0]}", "cityTo": f"{json['Origin'][0]}", "cityCodeTo": f"{json['Origin'][0]}",
                           ...
                           "airlines": ["null"], "route": [
            {"id": "null", "combination_id": "null", "flyFrom": f"{json['Origin'][0]}", "flyTo": f"{json['Origin'][0]}",
             "cityFrom": f"{json['Origin'][0]}", "cityCodeFrom": f"{json['Origin'][0]}", "cityTo": f"{json['Origin'][0]}",
             "cityCodeTo": f"{json['Origin'][0]}", "local_departure": time_from_1, "utc_departure": time_from_1,
            ...
            {"id": "null", "combination_id": "null", "flyFrom": f"{json['Origin'][0]}", "flyTo": f"{json['Origin'][0]}",
             "cityFrom": f"{json['Origin'][0]}", "cityCodeFrom": f"{json['Origin'][0]}", "cityTo": f"{json['Origin'][0]}",
             "cityCodeTo": f"{json['Origin'][0]}", "local_departure": time_to_1, "utc_departure": time_to_1,
    ...
    data_2['data'].append({"id": "null", "flyFrom": f"{json['Origin'][1]}", "flyTo": f"{json['Origin'][1]}", "cityFrom": f"{json['Origin'][1]}",
            ...
            {"id": "null", "combination_id": "null", "flyFrom": f"{json['Origin'][1]}", "flyTo": f"{json['Origin'][1]}",
             ...
             "cityCodeTo": f"{json['Origin'][1]}", "local_departure": time_from_2, "utc_departure": time_from_2,
            ...
            {"id": "null", "combination_id": "null", "flyFrom": f"{json['Origin'][1]}", "flyTo": f"{json['Origin'][1]}",
             "cityFrom": f"{json['Origin'][1]}", "cityCodeFrom": f"{json['Origin'][1]}", "cityTo": f"{json['Origin'][1]}",
             "cityCodeTo": f"{json['Origin'][1]}", "local_departure": time_to_2, "utc_departure": time_to_2,
    ...
    for objeto in range(len(data_1['data'])):
        if data_1['data'][objeto]['availability']['seats'] is not None:
            if data_1['data'][objeto]['availability']['seats'] >= json["Travelers"][0]:
    ...
    for objeto in range(len(data_2['data'])):
        if data_2['data'][objeto]['availability']['seats'] is not None:
            if data_2['data'][objeto]['availability']['seats'] >= json["Travelers"][1]:
    ...
    while n < n_max:
        ...
        total_passengers = json["Travelers"][0] + json["Travelers"][1]
